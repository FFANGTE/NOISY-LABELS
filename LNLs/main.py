...
os.makedirs('./results/cyclic/analysis', exist_ok=True)
...
for epoch in range(args.start_epoch, args.epochs + args.num_cycles * args.cycle_interval + 1):
    ...
    # softmax, loss = f.filter(model, ema_model)  # get predictions and losses by student model
    # analysis_dict['softmax'].append(deepcopy(softmax))
    # analysis_dict['loss_history'].append(deepcopy(loss))
    # preds = np.argmax(softmax, 1) == trainset.targets
    # clean_acc = 100. * sum(preds[clean_idxs_train]) / len(preds[clean_idxs_train])
    # noisy_acc = 100. * sum(preds[noisy_idxs_train]) / len(preds[noisy_idxs_train])
    # accuracy['train_clean_acc'].append(clean_acc)
    # accuracy['train_noisy_acc'].append(noisy_acc)

    # plot
    plt.figure()
    # plt.plot(range(epoch), accuracy['train_clean_acc'],
    #          color='midnightblue', label='Train_clean_acc')
    # plt.plot(range(epoch), accuracy['train_noisy_acc'],
    #          color='cornflowerblue', label='Train_noisy_acc')
